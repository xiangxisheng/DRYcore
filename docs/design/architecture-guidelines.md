...
为了确保系统架构的清晰性和可维护性，必须严格遵循以下模块定义和权限配置规范：

### 核心层和应用层职责界限

1. **核心层不应包含特定应用的业务模块**
   - 核心层（`src/core`、`src/config`等非apps目录）只能包含通用的框架功能
   - 特定业务模块（如云服务器、域名管理等）必须放在应用层（`src/apps/应用名称/`）中定义

2. **模块定义的正确位置**
   - 通用模块定义（如SYSTEM）应放在核心层的`config/module-base.ts`
   - 特定应用模块定义应放在对应应用目录下，如`apps/feieryun/shared/config/modules.ts`
   - 禁止在核心层定义特定应用的模块（例如SERVER、DOMAIN等）

3. **配置注册机制**
   - 应用层通过核心层提供的注册机制注册自己的配置
   - 核心层不应直接导入或依赖应用层的配置
...
```typescript
// 错误：在核心层的module-base.ts中定义特定应用模块
export const MODULE = {
  // 这是正确的：通用系统模块
  SYSTEM: { ... },
...
// 应用层：定义特定应用模块
// apps/feieryun/shared/config/modules.ts
